...
`hier_create()` allows to create a hierarchy. Argument `tot_lab` specifies the name of the root node. Optionally, it is possible to add some nodes to the top-level by listing their names in argument `node_labs`.

```{r}
h <- hier_create(tot_lab = "Total", node_labs = LETTERS[1:5])
print(h)
```
...
hier_add(h, refnode = "A", node_labs = c("a1", "a2"))
hier_add(h, refnode = "B", node_labs = c("b1", "b2"))
hier_add(h, refnode = "b1", node_labs = c("b1_a", "b1_b"))

# deleting one or more nodes from the hierarchy
hier_delete(h, node_labs = c("a1", "b2"))
hier_delete(h, node_labs = c("a2"))

# rename nodes
hier_rename(h, node_labs = c("C", "D"), node_labs_new = c("X", "Y"))
print(h)
```
...
Function `hier_info()` returns information about the nodes that are specified in argument `node_labs`. 
```{r}
# about a specific node
info <- hier_info(h, node_labs = c("b1", "E"))
```
...
Function `hier_convert()` takes a sdc hierarchy and allows to convert the tree-based structure to different formats while `hier_export()` does the conversion and writes the results to a file on the disk. The following formats are currently supported:
...
res_df <- hier_convert(h, format = "df")
print(res_df)
```

The required code to create this hierarchy could be computed using:
```{r}
res_code <- hier_convert(h, format = "code"); cat(res_code, sep = "\n")
```
...
```{r, eval=FALSE}
hier_export(h, format = "argus", path = "hierarchy.hrc")
```

## Create a hierarchy from data.frames, code or json

`hier_import()` returns a sdc hierarchy given either a data.frame (in `@;labs`-format), json format, code or from a tau-argus compatible `hrc-file`. For example if we want to create a hierarchy based of `res_df` which was previously created using `hier_convert()`, the code is as simple as:
...
identical(
  hier_convert(v1, format = "df"), 
  hier_convert(v2, format = "df")
)
print(v1)
```
...
We also note that there is another function `hier_compute_from_list()` that creates a hierarchy from a given named list. In such a list the name of a list element is interpreted as parent to all codes or labels listed for this element. A simple example is shown below:
...
d <- hier_compute_from_list(dim = yae_ll, tot_lev = "Total"); d
```

## Interactively create or modify hierarchies
The package also contains a shiny-app that can be started using `sdcHier()`. The app allows to pass as input either a character vector (that should be converted into a hierarchy) or an existing hierarchy and can be started as follows given the hierarchy previously generated using `hier_compute_fromList()`:
