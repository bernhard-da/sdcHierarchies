...
We also note that there is another function `sdcHier_compute_fromList()` that creates a hierarchy from a given named list. In such a list the name of a list element is interpreted as parent to all codes or labels listed for this element. A simple example is shown below:
...
The package also contains a shiny-app that can be started using `sdcHier()`. The app allows to pass as input either a character vector (that should be converted into a hierarchy) or an existing hierarchy and can be started as follows given the hierarchy previously generated using `sdcHier_compute_fromList()`:
