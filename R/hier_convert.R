...
hier_convert <- function(h, format="df", verbose=FALSE) {
  # to data.frame
  ...
  h_to_code <- function(h, verbose) {
    all_names <- hier_nodenames(h)
    code <- "library(sdcHierarchies)"
    t <- shQuote(all_names[1])
    code <- c(code, paste0("d <- hier_create(tot_lab = ", t, ")"))
    all_names <- all_names[-c(1)]

    if (length(all_names) > 0) {
      info <- hier_info(h, node_labs = all_names)
      while (length(all_names) > 0) {
        ...
        s3 <- paste0("hier_add(d, refnode = ", s1, ", node_labs = ", s2, ")")
        code <- c(code, s3)
      }
    }
    code <- c(code, "print(d)")
  ...
  h_to_argus <- function(h, verbose) {
    dforig <- df <- hier_convert(h, format = "df")
    df <- df[-1, ]
  ...
  h_to_sdc <- function(h, verbose) {
    ...
    all_info <- hier_info(h, node_labs = NULL)
