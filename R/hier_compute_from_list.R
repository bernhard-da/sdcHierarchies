#' Create a hierarchy from a named list
#'
#' This function allows to create a hierarchy from a list input. The name
#' of each list-element refers to a (sub)-total (node) in the hierarchy
#' while the values of the each list-elements to the leafs contributing
#' to this (sub)total (node).
#'
...
#' ); d
#'
#' ## second example; same as in ?hier_compute
#' yae_ll <- list()
...
#' d <- hier_compute_from_list(dim=yae_ll, tot_lev="Total", as_df=FALSE); d
hier_compute_from_list <- function(dim, tot_lev, as_df=FALSE) {
